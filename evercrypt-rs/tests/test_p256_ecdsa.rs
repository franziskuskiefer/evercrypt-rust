...
// TODO: test p256_ecdsa_sign
use evercrypt::p256::{EcdsaSignature, p256_ecdsa_verify, Error};
...
fn test_wycheproof() {
    ...
    for testGroup in tests.testGroups.iter() {
        ...
        let pk_x = make_fixed_length(&hex_str_to_bytes(&testGroup.key.wx));
        let pk_y = make_fixed_length(&hex_str_to_bytes(&testGroup.key.wy));
        let mut pk = Vec::new();
        pk.extend(&pk_x);
        pk.extend(&pk_y);
        ...
        for test in testGroup.tests.iter() {
            ...
            if test.tcId == 285 || // k*G has a large x-coordinate
                test.tcId == 339 // point duplication during verification
            {
                tests_skipped += 1;
                continue;
            }
